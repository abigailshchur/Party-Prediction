...
class Unigram_Classifier_DB:
    ...
    def get_events_cache(self):
        return self.classifier_meta_event_cache
...
def get_tweets_for_a_hashtag(hashtag, num_tweets = 10, views = ['text']):
    ...
    if cur_key >= len(keys):
        cur_key = 0
    qr = tweepy.Cursor(api.search, q=hashtag).items(num_tweets)
    out_list = []
    for twit in qr:
...
if __name__ == '__main__':
    ...
    ## going through each hashtag, getting all tweeets, classifying, putting them in DB ##
    tweet_lst = []
    for event in unigram_classifier_db.get_events_cache():
        # might be worth thresholding
        if len(unigram_classifier_db.get_events_cache()[event]['republicans']) > 30:
            tweets  = get_tweets_for_a_hashtag(event, num_tweets = 100, views = ['text', 'author'])
            for tweet in tweets:
                score = classifier.calculate_score(tweet['text'].lower().split(), event)
                ## and then we need to add tthis score and tweet to tweet_lst ##
